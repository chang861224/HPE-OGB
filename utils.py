...
def SplitDataset(dataset, percent=10):
    ...
    for node in sample_instances:
        ...
        sample_set["pos_target"].append(random.sample(edgelist[node], 1))
...
if __name__ == "__main__":
    pos = [0.3, 1]
    neg = [[0.5, 0.11, 0.2, 0.8], [0.3, 0.5, 0.1, 0.9]]
    print("%.4f" % float(torch.mean(Evaluate({"y_pred_pos": torch.tensor(pos), "y_pred_neg": torch.tensor(neg)}))))
